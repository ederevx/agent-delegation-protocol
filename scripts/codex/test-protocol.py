...
def test_round_robin_delegation_queue(home: Path) -> None:
    ...
    state_path = home / ".delegation-protocol" / "hook-state" / f"{session}.json"
    ...
    call_hook(home, "subagent-start", {
        "session_id": session, "turn_id": "t1", "agent_id": "virtual-a",
    })
    allowed = call_hook(home, "pretool", {
        "session_id": session, "turn_id": "t1", "tool_name": "apply_patch",
        "tool_input": {},
    })
...
def test_queue_fallbacks(root: Path) -> None:
    for condition in ("invalid", "unavailable", "misconfigured"):
        ...
        call_hook(home, "subagent-start", {
            "session_id": session, "turn_id": "t1", "agent_id": "only-worker",
        })
