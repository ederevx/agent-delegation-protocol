...
def runtime_contract(output: dict[str, Any]) -> str:
    ...
    if output["format"] == "claude-markdown":
        return (
            f"This profile's native agentic-turn limit is {limit} through "
            "maxTurns; deliver your final evidence report before reaching it."
        )
    if output["format"] == "pi-markdown":
        return (
            f"This profile carries a native budget of {limit} agentic turns "
            "enforced by the ADP subagent extension, which stops the worker "
            "at the limit; the worker must have delivered its final evidence "
            "report before then. ADP separately enforces "
            f"{limit} tool-call attempts per identified worker lifetime "
            "through the Pi delegation enforcer extension, including "
            "attempts later denied (defense in depth). Tool calls are not "
            "agentic turns."
        )
    return (
...
def render_pi(body: str, description: str, output: dict[str, Any]) -> str:
    ...
    if thinking not in ("low", "medium", "high", "xhigh"):
        raise ValueError(f"invalid Pi thinking level: {thinking!r}")
    marker = "\n## Host lifecycle\n"
    if body.count(marker) != 1:
        raise ValueError("Pi body must contain exactly one host lifecycle section")
    body = body.replace(
        marker,
        "\n## Thinking level\n\n"
