...
def main():
  with tempfile.TemporaryDirectory(prefix="claude-v2-") as raw:
    ...
    for hook_env in (env, inherited_env):
      ...
      for name in ('Agent', 'Task'):
        assert invoke('pre-mutation', dict(worker_payload, tool_name=name), hook_env) == {}, name
    ...
    # Recursive delegation is left to the model's own judgment, not
    # hook-enforced: a worker's own Agent/Task call is never denied for
    # naming any tier as its target, regardless of the caller's own tier.
    ...
    # The parent (non-worker session, no agent_id) is unaffected either way:
    # it may spawn any tier freely, as before.
    for target in ('bulk-worker', 'balanced-worker'):
