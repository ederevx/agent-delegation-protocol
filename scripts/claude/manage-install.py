...
def paths(repo: Path, home: Path) -> list[tuple[Path, Path]]:
    ...
    if os.environ.get("CI_CLAUDE_FOREGROUND_ONLY", "0") == "1":
        worker = repo / "claude" / "agents" / "ci-foreground-bulk-worker.md"
...
def uninstall(repo: Path, home: Path, python_exe: str) -> None:
    ...
    owned.extend([
        (repo / "claude/agents/bulk-worker.md", home / "agents/bulk-worker.md"),
        (repo / "claude/agents/ci-foreground-bulk-worker.md",
         home / "agents/bulk-worker.md"),
    ])
    owned.extend([
