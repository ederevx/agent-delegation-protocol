...
subprocesses here keeps all three suites covered by the same `pytest`
...
def test_pi_protocol_harness() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts/pi/test-protocol.py")],
        capture_output=True, text=True,
    )
    assert result.returncode == 0, result.stderr
