 ...
 * pinned to the bottom of the frame, the dim border rule above them, and
 * the content-window height arithmetic shared by render, wheel, and page
 * math. Frame contract: title (1 line) + content window (absorbs all
 * slack) + border (1 line) + kept instruction lines emit EXACTLY `rows`
 * lines for every rows ≥ 3; tiny terminals shrink the window to its floor
 * of 1 first, then drop instruction lines from the TOP (the tail is kept).
 */
...
export class ViewerChrome {
	...
	/** Instruction lines kept at `rows`. They pin to the frame's bottom:
	 ...
	 * to its floor of 1 first, then instruction lines drop from the TOP
	 * (the tail is kept); clipFrame is a rows<3-only final safety. */
	...
	/** Append the pinned bottom block at `rows`: the dim border rule, then
	 * exactly the kept instruction lines (consistent with the height math
	 * above, so the assembled frame totals `rows` lines before clipFrame). */
	appendBottom(out: string[], width: number, rows: number): void {
