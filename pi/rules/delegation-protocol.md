...
`main` first, then drop the stale branch. Merging into `main` and
force-pushing are allowed only under the explicit, single-use, text-based
authorization of Owner bypass below, granted for that exact operation and
consumed by it; without such authorization, never push to or merge directly
into `main` — reconcile through a PR and let the user land it.
