...
Pass the resulting absolute task path to the installed `.delegation-protocol/mux-scheduler.py <run|queue> --task-file <absolute-task-path> --route bulk --runtime claude` using Python 3. Pass the matching `--mode read|edit`, `--workspace shared|isolated`, and `--require audit|edit` capability filters. The installed file is under the isolated ci-claude config directory. The mux-scheduler selects an enabled backend by required capabilities and route priority; do not choose a provider yourself. Start it exactly once.

Invoke the mux-scheduler once through foreground `Bash` with `timeout: 600000`. Do not redirect or background the command: the owning ci-claude session must remain paused while its worker uses the single provider lane. If the command cannot finish within that ceiling, report the exact failure without replaying the task.
...
Classify the foreground command's receipt before interpreting its exit status. Never start a second mux-scheduler process for the same assignment.
