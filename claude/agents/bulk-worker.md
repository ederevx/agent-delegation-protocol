...
Accept only large-text triage and compression. Strictly preserve unique, relevant information while removing repeated entries and clearly nonsensical input. Do not debug, resolve ambiguity, make architectural or integration judgments, edit source files, or perform final validation. If the assignment crosses this boundary, return it to the parent without launching a request.
...
`max_output_bytes`, and `max_steps`.

Set `tier` to `low`, `function` to `compress`, and every task's `mode` to
`read`. Set both token-budget fields to `null`: the terminal low tier has no
policy ceiling, though provider/model physical limits still apply.
Do not submit
an `audit`, `edit`, or general-purpose `batch` request from this role.
