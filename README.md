...
The protocol enforces native delegation and worker budgets on Codex and
Claude.
...
Pi budget enforcement moved to pi-delegation, the Pi host repository
(private), where the tier budgets, the tool-call ledger, and the enforcer
extension own the whole mechanism in-process with no Python bridge.

The Pi host and its policy, generated profiles, and subagent extension were
extracted into a private repository named pi-delegation; this repository
keeps the shared classifier, installer engine, hook adapter, and the Codex
and Claude hosts.
...
classifier and hook adapter. The Pi host lives in the separate pi-delegation
repository. Manifest release metadata is retained for legacy
...
The Pi subagent extension moved with the rest of the Pi host into the
pi-delegation repository. There it remains vendored from
@earendil-works/pi-coding-agent `examples/extensions/subagent` (v0.85.1, MIT),
with every adaptation recorded in each file's provenance header.
