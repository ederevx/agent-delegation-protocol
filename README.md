...
The Pi subagent extension (`pi/extensions/adp-subagent/`) is vendored from
@earendil-works/pi-coding-agent `examples/extensions/subagent` (v0.85.1, MIT)
and maintained by this repository; the delegation enforcer is folded into
the same extension. Every adaptation (model frontmatter removed for
host-neutral deployment, native `maxTurns` turn budgets, full-screen TUI
detail viewer) is recorded in each file's provenance header. The four
hand-written example profiles that shipped with the vendored example
(planner, reviewer, scout, worker) have been retired; the deployed pi
worker profiles (`pi/agents/*-worker.md`) are generated by
`scripts/agents/render-bulk-workers.py` and are not hand-edited. The
