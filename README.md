...
The Pi subagent extension (`pi/extensions/subagent/`) is vendored from
@earendil-works/pi-coding-agent `examples/extensions/subagent` (v0.85.1, MIT)
and maintained by this repository. Every adaptation (model frontmatter removed
for host-neutral deployment, native `maxTurns` turn budgets, one-line TUI
output) is recorded in each file's provenance header. The four hand-written
example profiles that shipped with the vendored example (planner, reviewer,
scout, worker) have been retired; the deployed pi worker profiles
(`pi/agents/*-worker.md`) are generated by
